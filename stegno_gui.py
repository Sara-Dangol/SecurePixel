...
class SecureSteganographySystem:
    ...
    def init_database(self):
        ...
        try:
            ...
            # Check if salt column exists in users table, add if not
            cursor.execute("SHOW COLUMNS FROM users LIKE 'salt'")
            salt_column_exists = cursor.fetchone() is not None
            cursor.fetchall()
            ...
            if not salt_column_exists:
                try:
                    ...
                    print(f"Note: salt column may already exist or couldn't be added: {e}")
            ...
            # Check and add missing columns to secure_messages
            columns_to_add = [
                ('attachment_name', 'VARCHAR(255)'),
                ('attachment_size', 'INT'),
                ('attachment_type', 'VARCHAR(100)'),
            ...
            if admin_count == 0:
                ...
                kdf = hashlib.pbkdf2_hmac('sha256', "admin123".encode('utf-8'), salt, 100000, 32)
                ...
                cursor.execute('''
                    INSERT INTO users (username, email, password_hash, full_name, role, public_key, private_key_encrypted, salt)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                ''', ('admin', 'admin@steganography.com', password_hash, 'System Administrator', 'admin', 
                     public_key, private_key_encrypted, salt_b64))
            ...
            for username, email, full_name, role in test_users:
                ...
                if user_count == 0:
                    ...
                    salt = secrets.token_bytes(16)
                    salt_b64 = base64.b64encode(salt).decode('utf-8')
                    kdf = hashlib.pbkdf2_hmac('sha256', "password123".encode('utf-8'), salt, 100000, 32)
                    cipher = AES.new(kdf, AES.MODE_GCM)
                    ciphertext, tag = cipher.encrypt_and_digest(private_key.encode('utf-8'))
                    private_key_encrypted = json.dumps({
                        'ciphertext': base64.b64encode(ciphertext).decode('utf-8'),
                    ...
                    cursor.execute('''
                        INSERT INTO users (username, email, password_hash, full_name, role, public_key, private_key_encrypted, salt)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    ''', (username, email, password_hash, full_name, role, public_key, private_key_encrypted, salt_b64))
        ...
        try:
            ...
            cursor.execute('''
                SELECT id, username, email, password_hash, full_name, role, 
                       public_key, certificate_data, certificate_revoked, is_active, salt
                FROM users 
            ...
            if user:
                ...
                public_key, cert_data, cert_revoked, is_active, salt = user
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO users (username, email, password_hash, full_name, organization, 
                                 public_key, private_key_encrypted, security_question, security_answer_hash, salt)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ''', (user_data['username'], user_data['email'], password_hash, 
                 user_data['full_name'], user_data['organization'],
                 public_key, private_key_encrypted,
                 self.security_question.get(), security_answer_hash, salt_b64))
    ...
    def sign_document_action(self):
        ...
        try:
            ...
            # Get the salt and encrypted private key
            cursor.execute('SELECT private_key_encrypted, salt FROM users WHERE id = %s', 
                         (self.current_user['id'],))
            result = cursor.fetchone()
            cursor.fetchall()
            ...
            if not salt_b64:
                messagebox.showerror("Error", "No salt found for user. Please re-register.")
                cursor.close()
                return
            
            encrypted_data = json.loads(private_key_encrypted)
...
• admin / admin123"""
    ...
    def decrypt_message_action(self, message=None):
        ...
        try:
            ...
            private_key_encrypted = key_result[0]
            salt_b64 = key_result[1]
            
            if not salt_b64:
                messagebox.showerror("Error", "No salt found for user")
                cursor.close()
                return
            
            encrypted_key_data = json.loads(private_key_encrypted)
            salt = base64.b64decode(salt_b64)
        ...
        def create_user():
            ...
            try:
                ...
                ''', (fields['username'].get(), fields['email'].get(), password_hash,
                     ...
                     role_var.get(), active_var.get(), public_key, private_key_encrypted, salt_b64))
